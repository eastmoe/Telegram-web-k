...
将 `config.example.json` 复制为 `config.json`，填写服务器端口、访问认证和可选的上游 HTTP 代理，然后运行：
...
```json
{
  ...
  "auth": {
    "enabled": true,
    "username": "admin",
    "passwordHash": "scrypt:生成的盐值:生成的密码摘要"
  },
  "telegram": {
    "proxyEnabled": true,
...
`auth.enabled` 控制 Web 界面、HTTP API 和 WebSocket 的访问认证。运行 `node server/hashPassword.js <password>` 可以生成 `auth.passwordHash`。浏览器会将登录会话保存在 localStorage 中，API 客户端也可以使用 HTTP Basic Auth。
...
生产服务直接读取 `docker-compose.yaml` 中的环境变量，并使用已发布的 GHCR 镜像：
...
| `HTTP_AUTH_ENABLED` | `false` | 控制 Web 界面、API 和 WebSocket 的访问认证。 |
| `HTTP_AUTH_USERNAME` | `admin` | 设置访问认证用户名。 |
| `HTTP_AUTH_PASSWORD` | 空 | 设置 Docker 登录密码，容器启动时会生成 scrypt 哈希。 |
...
运行 `docker compose up tweb.production -d` 拉取并启动 WebK 服务器与 Telegram 中继。默认配置访问 http://localhost:8080/；端口 `443` 的直接 TLS 配置使用对应的 `https://` 主机名。
